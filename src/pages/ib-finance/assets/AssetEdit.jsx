...
const AssetEdit = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          asset_type: asset.asset_type || 'tangible',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate(`/ib-finance/assets/${id}`);
      } else {
        const err = await response.json();
        toast.error(err.detail || 'Failed to update asset');
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50" data-testid="asset-edit">
      <div className="bg-white border-b border-gray-200">
        <div className="px-8 py-6">
          <div className="flex items-center gap-4 mb-4">
            <button onClick={() => navigate(`/ib-finance/assets/${id}`)} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg">
              <ArrowLeft className="h-5 w-5" />
            </button>
            <span className="text-sm text-gray-500">IB Finance → Assets → Edit</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <button
                type="button"
                onClick={() => navigate(`/ib-finance/assets/${id}`)}
            ...
            <div className="bg-amber-50 rounded-xl border border-amber-200 p-4">
              <p className="text-sm text-amber-800">
                <strong>Note:</strong> Capitalization value and acquisition date cannot be changed after asset creation. Contact finance admin for corrections.
              </p>
            </div>
          </div>
        </div>
