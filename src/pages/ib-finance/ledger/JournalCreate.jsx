...
const JournalCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = authService.getToken();
      const response = await fetch(`${API_URL}/api/ib-finance/ledger/journals`, {
        method: 'POST',
        headers: { 
      ...
      if (response.ok) {
        ...
        navigate(`/ib-finance/ledger/journal/${data.data.journal_id}`);
      } else {
        const err = await response.json();
        toast.error(err.detail || 'Failed to create journal');
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50" data-testid="journal-create">
      <div className="bg-white border-b border-gray-200">
        <div className="px-8 py-6">
          <div className="flex items-center gap-4 mb-4">
            <button onClick={() => navigate('/ib-finance/ledger')} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg">
              <ArrowLeft className="h-5 w-5" />
            </button>
            <span className="text-sm text-gray-500">IB Finance → Ledger → New Journal</span>
          </div>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Record a manual journal entry</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <button
                type="button"
                onClick={() => navigate('/ib-finance/ledger')}
              ...
              <p className="text-xs text-gray-500 mt-4 text-center">
                Journal will be created in Draft status. Post it to finalize.
              </p>
            </div>
          </div>
        </div>
